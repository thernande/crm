...
var Bootstrappifier = {
    ...
    labels: function(){
      $('.required input.form-error').each(function(){
        $('[for=' + $(this).attr('id') + ']').addClass('required');
      });
      $('.required textarea.form-error').each(function(){
        $('[for=' + $(this).attr('id') + ']').addClass('required');
      });
    },
    remove_control_error: function(){
      $('[class="controls required error"]').each(function(){

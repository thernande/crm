...
<p>
	...
	echo $this->Html->link("Crear Nueva Oportunidad", array(
			'action' => 'add'), array(
			'height'=>'15',
			'width' =>'15',
...
jQuery(document).ready(function($){
	jQuery("#list").jqGrid({
   	url:'<?php echo $this->Html->url(array("controller" => "oportunities", "action" => "showGridOpor")); ?>',
	datatype: "json",
	mtype: "GET",
   	...
   	colModel:[
   	   {name:'id',index:'id',hidden:true,width:10},
   		...
   		{name: 'action', width:100, fixed:true, frozen:true, sortable:false,formatter:'actions', resize:false, formatoptions:{editbutton:false,delbutton:true,keys:true,delOptions:{
       url:'<?php echo $this->Html->url(array("controller" => "oportunities", "action" => "delete"));?>',
        mtype: "POST",
                      onclickSubmit :function(params, postdata) {
                        params.url = '<?php echo $this->Html->url(array("controller" => "oportunities", "action" => "delete"));?>'+'/'+postdata ;
                      }
        }
        }}
   	],
    ...
    sortorder: 'asc',
    loadonce: true,
	caption: "Administracion de Oporunidades de Negocio",
	height:"auto",
     autowidth:true,     
            ...
            $("<div>",
                {
                    ...
                    click: function(e) {
                        document.location.href = "oportunities/edit/"+$(e.target).closest("tr.jqgrow").attr("id");
                    }
                }
    ...
    grid.children("tbody")
            ...
            $("<div>",
                {
                    ...
                    click: function(e) {
                    	var answer = confirm ("¿Quieres Aprobar la Propuesta?")
						if (answer)
							document.location.href = "oportunities/state/"+$(e.target).closest("tr.jqgrow").attr("id");                      
                    }
                }
              ).css({"margin-left": "5px", float:"left"})
               .addClass("ui-pg-div ui-inline-custom")
     ...
     ondblClickRow: function (rowid) {
        ...
        document.location.href = "oportunities/view/" + rowData['id'];
    }
	});
